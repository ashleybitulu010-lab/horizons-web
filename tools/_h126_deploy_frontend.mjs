#!/usr/bin/env node
/**
 * H12.6 — Build and deploy frontend with migrated READ flags (Sales+Expenses+Stock+Products+Debts+Profit).
 */
...
const buildOut = path.join(webRoot, '.dist-h126');
...
async function main() {
  ...
  console.log('Connected to VPS (H12.6 deploy)');
  ...
  const ts = Date.now();
  const remoteTar = `/tmp/ashledger-dist-h126-${ts}.tar.gz`;
  const localTar = path.join(webRoot, `.dist-h126-${ts}.tar.gz`);
...
set -euo pipefail
BACKUP="${APP_DIR}/.dist-backup-h126-${ts}"
...
`);
  ...
  writeFileSync(path.join(webRoot, '.dist-h126-deploy.json'), JSON.stringify({
    phase: 'H12.6',
    flags: env,
    bundle: remoteBundle,
    ts,
  ...
  rmSync(buildOut, { recursive: true, force: true });
  console.log('H12.6 frontend deploy complete.', remoteBundle);
}

main().catch((e) => {
  console.error('H12.6 deploy failed:', e.message);
  process.exit(1);
});
