#!/usr/bin/env node
/**
 * H12.5 — Deploy Debts READ template fix to VPS API container.
 */
...
const FILES = [
  ...
  'tests/intelligence-v2-response.test.js',
  'tests/intelligence-v2-financial.test.js',
];
...
async function main() {
  ...
  try {
    ...
    const marker = await sshExec(
      conn,
      `docker exec ${CONTAINER} node -e "import('./src/agent/intelligence-v2/response/response-templates.js').then(m=>console.log(typeof m.templateDebtsRetrieve))"`,
    );
    console.log('templateDebtsRetrieve:', marker);
    console.log('H12.5 API deploy complete.');
  } finally {
    conn.end();
  }
}
...
main().catch((e) => {
  console.error('H12.5 API deploy failed:', e.message);
  process.exit(1);
});
