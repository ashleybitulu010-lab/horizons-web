/**
 * Apply F4-B2-C migration to Supabase STAGING only.
 * Usage: node --env-file=apps/api/.env.staging tools/_apply_f4_b2_c_migration.mjs
 */
...
async function main() {
	...
	const sqlPath = path.join(__dirname, '..', 'supabase', 'migrations', '20260909140000_confirm_and_create_idempotent_b2c.sql');
	...
	try {
		...
		console.log('F4-B2-C idempotent confirm_and_create migration applied to staging');
	} finally {
		await client.end();
	}
}
