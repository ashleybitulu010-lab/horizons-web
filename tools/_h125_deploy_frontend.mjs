#!/usr/bin/env node
/**
 * H12.5 — Build and deploy frontend with migrated READ flags (Sales+Expenses+Stock+Products+Debts).
 */
...
const buildOut = path.join(webRoot, '.dist-h125');
...
async function main() {
  ...
  console.log('Connected to VPS (H12.5 deploy)');
  ...
  const ts = Date.now();
  const remoteTar = `/tmp/ashledger-dist-h125-${ts}.tar.gz`;
  const localTar = path.join(webRoot, `.dist-h125-${ts}.tar.gz`);
...
set -euo pipefail
BACKUP="${APP_DIR}/.dist-backup-h125-${ts}"
...
`);
  ...
  writeFileSync(path.join(webRoot, '.dist-h125-deploy.json'), JSON.stringify({
    phase: 'H12.5',
    flags: env,
    bundle: remoteBundle,
    ts,
  ...
  rmSync(buildOut, { recursive: true, force: true });
  console.log('H12.5 frontend deploy complete.', remoteBundle);
}

main().catch((e) => {
  console.error('H12.5 deploy failed:', e.message);
  process.exit(1);
});
