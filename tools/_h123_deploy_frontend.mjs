#!/usr/bin/env node
/**
 * H12.3 — Build and deploy frontend with migrated READ flags (Sales+Expenses+Stock).
 */
...
const buildOut = path.join(webRoot, '.dist-h123');
...
async function main() {
  ...
  console.log('Connected to VPS (H12.3 deploy)');
  ...
  const ts = Date.now();
  const remoteTar = `/tmp/ashledger-dist-h123-${ts}.tar.gz`;
  const localTar = path.join(webRoot, `.dist-h123-${ts}.tar.gz`);
...
set -euo pipefail
BACKUP="${APP_DIR}/.dist-backup-h123-${ts}"
...
`);
  ...
  writeFileSync(path.join(webRoot, '.dist-h123-deploy.json'), JSON.stringify({
    phase: 'H12.3',
    flags: env,
    bundle: remoteBundle,
    ts,
  ...
  rmSync(buildOut, { recursive: true, force: true });
  console.log('H12.3 frontend deploy complete.', remoteBundle);
}

main().catch((e) => {
  console.error('H12.3 deploy failed:', e.message);
  process.exit(1);
});
