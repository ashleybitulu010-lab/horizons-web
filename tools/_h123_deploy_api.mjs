#!/usr/bin/env node
/**
 * H12.3 — Deploy Stock READ template fix to VPS API container.
 */
...
async function main() {
  ...
  try {
    ...
    const marker = await sshExec(
      conn,
      `docker exec ${CONTAINER} node -e "import('./src/agent/intelligence-v2/response/response-templates.js').then(m=>console.log(typeof m.templateStockRetrieve))"`,
    );
    console.log('templateStockRetrieve:', marker);
    console.log('H12.3 API deploy complete.');
  } finally {
    conn.end();
  }
}
...
main().catch((e) => {
  console.error('H12.3 API deploy failed:', e.message);
  process.exit(1);
});
