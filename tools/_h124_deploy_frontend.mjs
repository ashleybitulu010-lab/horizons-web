#!/usr/bin/env node
/**
 * H12.4 — Build and deploy frontend with migrated READ flags (Sales+Expenses+Stock+Products).
 */
...
const buildOut = path.join(webRoot, '.dist-h124');
...
async function main() {
  ...
  console.log('Connected to VPS (H12.4 deploy)');
  ...
  console.log('Build flags:', {
    VITE_ASHY_READ_SALES: env.VITE_ASHY_READ_SALES,
    VITE_ASHY_READ_EXPENSES: env.VITE_ASHY_READ_EXPENSES,
    VITE_ASHY_READ_STOCK: env.VITE_ASHY_READ_STOCK,
    VITE_ASHY_READ_PRODUCTS: env.VITE_ASHY_READ_PRODUCTS,
  });
  ...
  const ts = Date.now();
  const remoteTar = `/tmp/ashledger-dist-h124-${ts}.tar.gz`;
  const localTar = path.join(webRoot, `.dist-h124-${ts}.tar.gz`);
...
set -euo pipefail
BACKUP="${APP_DIR}/.dist-backup-h124-${ts}"
...
`);
  ...
  writeFileSync(path.join(webRoot, '.dist-h124-deploy.json'), JSON.stringify({
    phase: 'H12.4',
    flags: env,
    bundle: remoteBundle,
    ts,
  ...
  rmSync(buildOut, { recursive: true, force: true });
  console.log('H12.4 frontend deploy complete.', remoteBundle);
}

main().catch((e) => {
  console.error('H12.4 deploy failed:', e.message);
  process.exit(1);
});
