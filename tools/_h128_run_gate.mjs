#!/usr/bin/env node
/** Load ASH_TEST_PASSWORD from VPS .env and run H12.8 gate. */
...
async function main() {
  ...
  const gate = spawnSync(process.execPath, ['tools/_h128_report_text_migration_gate.mjs'], {
    cwd: webRoot,
    stdio: 'inherit',
...
main().catch((e) => {
  console.error('H12.8 gate runner failed:', e.message);
  process.exit(1);
});
