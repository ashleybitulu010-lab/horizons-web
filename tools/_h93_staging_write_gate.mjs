#!/usr/bin/env node
/**
 * H9.3-A — Staging write safety gate (vwjkktqcmmhotadicbpg only).
 * Does NOT touch production. H9.3-B requires GO_H9_3_B_READY from this gate.
 *
 *   node --env-file=apps/api/.env.staging tools/_h93_staging_write_gate.mjs
 */
...
async function createAuthUser(admin, label) {
	const email = `${TAG}-${label}-${randomUUID()}@h93.ashledger.test`;
	const { data, error } = await admin.auth.admin.createUser({
		email,
		password: `H93!${randomUUID().slice(0, 8)}`,
		email_confirm: true,
	});
...
async function main() {
	...
	const report = {
		...
		scenarios: {},
		notSimulated: ['H9.3-B production write gate', 'post-commit HTTP network loss', 'extended 30min monitoring'],
		verdict: 'NO_GO',
	};
	...
	try {
		...
		// --- Fixtures: Client A (A1, A2), Client B (B1) ---
		...
		// --- A3 CREATE EXPENSE (user message) ---
		...
		const depAfterProposal = (await snapshotCounts(admin, clientA, activityA1)).depenses;
		const awAfterProposal = (await snapshotCounts(admin, clientA, activityA1)).agent_write_operations;

		report.scenarios.expenseProposal = expProposal.status === 200
			&& expProposal.body?.v2Http?.goalType === 'ACTION'
			&& expProposal.body?.v2Http?.goalDomain === 'EXPENSES'
			&& expProposal.body?.v2Http?.actionProposalStatus === 'READY_FOR_CONFIRMATION'
			&& depAfterProposal === report.baseline.depenses
			&& awAfterProposal === report.baseline.agent_write_operations
			? pass('expense proposal', {
				goalType: expProposal.body?.v2Http?.goalType,
				actionStatus: expProposal.body?.v2Http?.actionProposalStatus,
			})
			: fail('expense proposal', { v2: expProposal.body?.v2Http, depAfterProposal, awAfterProposal });

		// --- A4 PENDING VALIDATION ---
		report.scenarios.pendingFields = pendingExp?.pending?.operationId
			...
			? pass('pending DB fields', {
				operationId: pendingExp.pending.operationId,
				stateVersion: pendingExp.stateVersion,
				hasConsumeToken: Boolean(pendingExp.pending.consumeToken),
			})
			: fail('pending fields', pendingExp);

		// --- A5 CONFIRM EXPENSE ---
		const opIdExp = pendingExp?.pending?.operationId;
		const expConfirm = await postChat(baseUrl, { message: 'Oui', sessionId: expSession, activityId: activityA1 });
		const depAfterConfirm = (await snapshotCounts(admin, clientA, activityA1)).depenses;
		...
		report.scenarios.expenseConfirm = expConfirm.status === 200
			&& (expConfirm.body?.v2Http?.actionProposalStatus === 'COMPLETED' || expConfirm.body?.v2Http?.f4Committed === true)
			&& depAfterConfirm === report.baseline.depenses + 1
			&& opExp?.status === 'completed'
			&& opExp?.result_id
			&& !pendingAfterExp?.pending?.pendingWrite
			? pass('expense F4-B2 confirm', {
				opStatus: opExp?.status,
				resultId: opExp?.result_id,
				depAfter: depAfterConfirm,
			})
			: fail('expense confirm', { expConfirm: expConfirm.body?.v2Http, opExp, depAfterConfirm });

		// --- A6 DOUBLE CONFIRMATION ---
		const doubleConfirm = await postChat(baseUrl, { message: 'Oui', sessionId: expSession, activityId: activityA1 });
		const depAfterDouble = (await snapshotCounts(admin, clientA, activityA1)).depenses;
		report.scenarios.doubleConfirm = depAfterDouble === report.baseline.depenses + 1
			...
			? pass('double confirm no extra write', { status: doubleConfirm.body?.v2Http?.actionProposalStatus })
			: fail('double confirm', { doubleConfirm: doubleConfirm.body?.v2Http, depAfterDouble });

		// --- A11 CREATE SALE ---
		const saleSession = `${TAG}-sale`;
		const saleProposal = await postChat(baseUrl, {
			message: "J'ai vendu 2 poulets à 30 dollars.",
			sessionId: saleSession,
			activityId: activityA1,
		});
		const pendingSale = await fetchPendingSession(admin, clientA, activityA1);
		const saleAfterProposal = (await snapshotCounts(admin, clientA, activityA1)).ventes;
		...
		const saleAfterConfirm = (await snapshotCounts(admin, clientA, activityA1)).ventes;
		const opSale = opIdSale ? await getWriteOperation(admin, opIdSale) : null;
		const saleDouble = await postChat(baseUrl, { message: 'Oui', sessionId: saleSession, activityId: activityA1 });
		const saleAfterDouble = (await snapshotCounts(admin, clientA, activityA1)).ventes;

		report.scenarios.saleE2E = saleProposal.status === 200
			&& saleProposal.body?.v2Http?.goalType === 'ACTION'
			&& saleProposal.body?.v2Http?.actionProposalStatus === 'READY_FOR_CONFIRMATION'
			&& saleAfterProposal === report.baseline.ventes
			&& saleConfirm.body?.v2Http?.f4Committed === true
			&& saleAfterConfirm === report.baseline.ventes + 1
			&& opSale?.status === 'completed'
			&& saleAfterDouble === report.baseline.ventes + 1
			? pass('sale E2E + double confirm', { saleAfterConfirm, opStatus: opSale?.status })
			: fail('sale E2E', { saleProposal: saleProposal.body?.v2Http, saleConfirm: saleConfirm.body?.v2Http, saleAfterConfirm });

		// --- A12 ACTIVITY ISOLATION ---
		...
		// --- A14 FALLBACK SAFETY (n8n) ---
		const n8nBlocked = expProposal.body?.v2Http?.noN8nFallback !== false
			&& saleProposal.body?.v2Http?.noN8nFallback !== false;
		report.scenarios.fallbackSafety = n8nBlocked
			? pass('ACTION no n8n fallback flag', { expense: expProposal.body?.v2Http?.noN8nFallback, sale: saleProposal.body?.v2Http?.noN8nFallback })
			: fail('fallback safety', { exp: expProposal.body?.v2Http, sale: saleProposal.body?.v2Http });
		...
		const advPending = { tool: 'create_expense', label: `H93 ${TAG}`, amount: 30 };
		...
		report.scenarios.idempotencyReplay = rpcFirst.data?.status === 'COMMITTED'
			...
			? pass('idempotency replay', { first: rpcFirst.data?.status, replay: rpcReplay.data?.status })
			: fail('idempotency replay', { rpcFirst: rpcFirst.data, rpcReplay: rpcReplay.data });

		// A8 request hash mismatch (new pending)
		const hashOpId = randomUUID();
		const hashToken = `tok-hash-${TAG}`;
		const hashPending2 = { tool: 'create_expense', label: `Hash ${TAG}`, amount: 40 };
		const goodHash = hashPending(clientAdv, hashPending2);
		await seedPending(admin, clientAdv, activityAdv, hashPending2, hashToken, hashOpId, 2);
		...
		report.scenarios.requestHash = hashCommit.data?.status === 'COMMITTED'
			&& hashMismatch.data?.status === 'REQUEST_HASH_MISMATCH'
			&& (await snapshotCounts(admin, clientAdv, activityAdv)).depenses === depBeforeHash + 1
			? pass('request hash mismatch blocked', { mismatch: hashMismatch.data?.status })
			: fail('request hash', { hashCommit: hashCommit.data, hashMismatch: hashMismatch.data });
		...
		// A10 stale version — bump DB version, confirm with stale expected_version (no valid token)
		...
		report.scenarios.noSecretLeak = replyHasNoSecrets(expConfirm.body?.reply)
			&& replyHasNoSecrets(saleConfirm.body?.reply)
			? pass('no secrets in reply')
			: fail('secret leak', { expReply: expConfirm.body?.reply?.slice(0, 80) });

		// --- A16 FINANCIAL INTEGRITY (main E2E client A1) ---
		...
		report.scenarios.financialIntegrity = report.financialDelta.depenses === 1
			&& report.financialDelta.ventes === 1
			&& report.financialDelta.agent_write_operations >= 2
			? pass('financial integrity main track', report.financialDelta)
			: fail('financial integrity', report.financialDelta);

		server.close();

		const allPass = Object.values(report.scenarios).every((s) => s.status === 'PASS');
		report.verdict = allPass ? 'GO_H9_3_B_READY' : 'NO_GO';
	} finally {
	...
	writeFileSync(`${process.env.TEMP}/h93_staging_report.json`, JSON.stringify(report, null, 2));
	console.log(JSON.stringify({ verdict: report.verdict, scenarios: report.scenarios, financialDelta: report.financialDelta }, null, 2));
	process.exit(report.verdict === 'GO_H9_3_B_READY' ? 0 : 1);
}
