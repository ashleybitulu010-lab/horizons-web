/**
 * Apply F4-B2-B migration to Supabase STAGING only.
 * Usage: node --env-file=apps/api/.env.staging tools/_apply_f4_b2_b_migration.mjs
 */
...
async function main() {
	...
	const sqlPath = path.join(__dirname, '..', 'supabase', 'migrations', '20260908140000_agent_write_operations.sql');
	...
	try {
		...
		console.log('F4-B2-B agent_write_operations migration applied to staging');
	} finally {
		await client.end();
	}
}
