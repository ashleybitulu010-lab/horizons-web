#!/usr/bin/env node
/**
 * H12.8 — Deploy Report text classifier fix to VPS API container.
 */
...
const FILES = [
  ...
  'tests/intelligence-v2-classifier-h128.test.js',
];
...
async function main() {
  ...
  try {
    ...
    console.log('H12.8 API deploy complete.');
  } finally {
    conn.end();
  }
}
...
main().catch((e) => {
  console.error('H12.8 API deploy failed:', e.message);
  process.exit(1);
});
