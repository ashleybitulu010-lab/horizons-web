#!/usr/bin/env node
/**
 * H12.8 — Build and deploy frontend with migrated READ flags (incl. Report text).
 */
...
const buildOut = path.join(webRoot, '.dist-h128');
...
async function main() {
  ...
  console.log('Connected to VPS (H12.8 deploy)');

  const envRaw = await sshExec(conn, `grep -E '^VITE_SUPABASE_(URL|ANON_KEY)=' ${APP_DIR}/.env 2>/dev/null || true`);
  const env = {
    VITE_ASHY_READ_SALES: String(process.env.VITE_ASHY_READ_SALES ?? 'true'),
    VITE_ASHY_READ_EXPENSES: String(process.env.VITE_ASHY_READ_EXPENSES ?? 'true'),
    VITE_ASHY_READ_STOCK: String(process.env.VITE_ASHY_READ_STOCK ?? 'true'),
    VITE_ASHY_READ_PRODUCTS: String(process.env.VITE_ASHY_READ_PRODUCTS ?? 'true'),
    VITE_ASHY_READ_DEBTS: String(process.env.VITE_ASHY_READ_DEBTS ?? 'true'),
    VITE_ASHY_READ_PROFIT: String(process.env.VITE_ASHY_READ_PROFIT ?? 'true'),
    VITE_ASHY_READ_REPORT: String(process.env.VITE_ASHY_READ_REPORT ?? 'true'),
  };
  ...
  const ts = Date.now();
  const remoteTar = `/tmp/ashledger-dist-h128-${ts}.tar.gz`;
  const localTar = path.join(webRoot, `.dist-h128-${ts}.tar.gz`);
  execSync(`tar -czf "${localTar}" -C "${buildOut}" .`, { stdio: 'inherit', shell: true });

  await sshUpload(conn, localTar, remoteTar);
  const remoteBundle = (await sshExec(conn, `
set -euo pipefail
BACKUP="${APP_DIR}/.dist-backup-h128-${ts}"
...
`);
  ...
  writeFileSync(path.join(webRoot, '.dist-h128-deploy.json'), JSON.stringify({
    phase: 'H12.8',
    flags: env,
    bundle: remoteBundle,
    ts,
  ...
  rmSync(buildOut, { recursive: true, force: true });
  console.log('H12.8 frontend deploy complete.', remoteBundle);
}

main().catch((e) => {
  console.error('H12.8 deploy failed:', e.message);
  process.exit(1);
});
