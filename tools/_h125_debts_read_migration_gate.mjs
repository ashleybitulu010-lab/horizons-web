#!/usr/bin/env node
/**
 * H12.5 — Debts READ migration gate (READ-only, no financial writes).
 */
...
const TAG = `h125-${Date.now()}`;
const REPORT_PATH = `${process.env.TEMP}/h125_report.json`;

const DEBTS_READ_MSGS = [
...
const report = {
  phase: 'H12.5',
  tag: TAG,
  flag: 'VITE_ASHY_READ_DEBTS=true',
  gitAudit: null,
  deploy: {},
  localTests: null,
  metricsBefore: null,
  ...
  errors: [],
  gaps: [
    'Combien ai-je payé? / Mes paiements — ambiguous, not migrated',
    'Multi-turn debtor follow-up (Et pour Jean?) — context gap documented',
  ],
};
...
async function main() {
  ...
  const apiDeploy = spawnSync(process.execPath, ['tools/_h125_deploy_api.mjs'], { cwd: webRoot, encoding: 'utf8' });
  report.deploy.api = { exitCode: apiDeploy.status, stderrTail: (apiDeploy.stderr || '').slice(-400) };
  if (apiDeploy.status !== 0) throw new Error('H125_API_DEPLOY_FAIL');

  const feDeploy = spawnSync(process.execPath, ['tools/_h125_deploy_frontend.mjs'], {
    cwd: webRoot,
    encoding: 'utf8',
    env: {
      ...process.env,
  ...
  if (feDeploy.status !== 0) throw new Error('H125_FRONTEND_DEPLOY_FAIL');
  ...
  try {
    ...
    let debtsCorrelation = null;
    for (let i = 0; i < DEBTS_READ_MSGS.length; i += 1) {
      const msg = DEBTS_READ_MSGS[i];
      const r = await chat(token, activityId, msg, `${TAG}-debts-${i}`);
      report.scenarios[`debtsRead_${i}`] = snap(r, msg);
      if (i === 0) debtsCorrelation = r.correlationId;
    ...
    const foreign = await chat(token, '00000000-0000-0000-0000-000000000099', DEBTS_READ_MSGS[0], `${TAG}-foreign`);
    ...
    report.correlation.debtsRead = debtsCorrelation;
    ...
    const debtScenarios = Object.entries(report.scenarios).filter(([k]) => k.startsWith('debtsRead_'));
    const debtsOk = debtScenarios.every(([, s]) => s.status === 200
      && s.goalDomain === 'DEBTS'
      && s.goalType === 'QUESTION'
    ...
    const debtsDelta = report.metricsDelta?.byCapability?.DEBTS?.READ_V2_SUCCESS ?? 0;
    ...
    report.counterCoherence = {
      ...
      debtsV2Delta: debtsDelta,
      actionFallbackDelta,
      controlledDebtsReads: DEBTS_READ_MSGS.length,
    };

    report.rollback = { localVerified: true, note: 'VITE_ASHY_READ_DEBTS=false → n8n (local tests)' };

    if (debtsOk && salesOk && expensesOk && stockOk && productsOk
      && report.scenarios.foreignActivity.pass
      && report.financialDeltaZero
      && actionFallbackDelta === 0
      && debtsDelta >= 1) {
      report.verdict = report.gaps.length ? 'H12.5_GO_WITH_GAPS' : 'H12.5_GO';
    }
  } finally {
    conn.end();
  }
  ...
  process.exit(report.verdict.startsWith('H12.5_GO') ? 0 : 1);
}
