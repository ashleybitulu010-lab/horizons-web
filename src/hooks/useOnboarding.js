...
Écrivez une seule information à la fois dans le chat principal ci-dessous.`,
...
Aucune syntaxe compliquée — écrivez comme vous parleriez.`,
...
Une seule dépense à la fois.`,
  ...
  },
  {
    ...
    explain: `Dernière étape : demandez votre bilan.

Écrivez dans le chat principal :
Génère mon bilan PDF`,
    success: `🎉 Félicitations !
Vous maîtrisez maintenant les bases d'Ash Ledger.

Ashy devient votre assistant financier permanent. Posez-moi vos questions quand vous voulez !`,
    check: 'rapport',
  },
];
...
Règle importante : une seule donnée à la fois dans le chat principal.`,
  actions: [
...
export const ORDER_RULE_MESSAGE = `Impossible d'effectuer une vente.
...
export const ONE_DATA_REMINDER = 'Rappel : saisissez une seule donnée à la fois dans le chat principal.';
...
export function progressFromState(state) {
  ...
  if (state.status === 'completed') {
    return { percent: 100, label: 'Terminé', step: ONBOARDING_STEPS.length, total: ONBOARDING_STEPS.length };
  }
  if (state.status !== 'active') {
    return { percent: 0, label: '', step: 0, total: ONBOARDING_STEPS.length };
  }
  const step = ONBOARDING_STEPS[state.stepIndex];
  if (!step) return { percent: 100, label: 'Terminé', step: ONBOARDING_STEPS.length, total: ONBOARDING_STEPS.length };
  return {
